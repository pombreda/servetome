...
{
	// Exit gracefuly instead of letting the unhandled exception trigger an
	// alert window.
	exit(1);
}
	...
	int flip_srt)
{
	...
	if (strncmp(subtitle_stream_specifier, "nil", 4) != 0)
	{
		long languageIndex;
...
{
	int jsonLength = fprintf(file,"\"length\":%f,\"audio_streams\":[",
...
static int outputMetadata(STMTranscodeContext *context)
{
...
int main(int argc, char **argv)
{
		...
		strcmp(argv[OPERATION_NAME], "metadata") == 0)
	{
		...
		outputMetadata(&transcode_context);
		result = 0;
		goto close_and_exit;
	}
