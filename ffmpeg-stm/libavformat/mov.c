...
static int mov_read_udta_string(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    if (parse)
		...
		char *atom_data = NULL;
		if (atom.size > str_size)
		{
			atom_data = av_malloc(atom.size);
			get_buffer(pb, (unsigned char *)atom_data, atom.size);
			strncpy(str, atom_data, str_size);
		}
		else
		{
        ...
        str[str_size] = 0;
        av_metadata_setdata(&c->fc->metadata, key, str, atom_data, atom.size, 0);
