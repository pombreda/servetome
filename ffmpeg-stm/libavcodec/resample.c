...
static void multi_to_stereo_split(short *output1, short *output2, short *input, int n, int channels) {
	...
	if (channels == 2) {
		...
		for(i=0;i<n;i++) {
			...
			/* output l & r. */
			*output1++ = clip_short(fl + (0.5 * rl) + (0.7 * c));
			*output2++ = clip_short(fr + (0.5 * rr) + (0.7 * c));
		...
		for(i=0;i<n;i++) {
			...
			int l = 0;
			int r = 0;
			for(j=0;j<channels-1;j+=2)
			{
				l += input[j];
				r += input[j+1];
			}
			*output1++ = clip_short(0.5 * l);
			*output2++ = clip_short(0.5 * r);
		}
	}
}
...
/* channels will always be >= 2.
 ...
 - Right = front_right + rear_gain * rear_right + center_gain * center
 where rear_gain is usually around 0.5-1.0 and center_gain is almost always 0.7 (-3 dB) if I recall correctly.  */
static void multi_to_mono_split(short *output1, short *input, int n, int channels) {
    int i;

	if (channels == 2) {
		for(i=0;i<n;i++) {
			/* output l & r. */
			*output1++ = clip_short(0.5 * input[0] + 0.5 * input[1]);
		...
		for(i=0;i<n;i++) {
			...
			/* output l & r. */
			*output1++ = clip_short(0.5 * fl + 0.5 * fr + 0.25 * rl + 0.25 * rr + 0.7 * c);
		...
		for(i=0;i<n;i++) {
			...
			*output1++ = clip_short(0.5 * l + 0.5 * r);
...
int audio_resample(ReSampleContext *s, short *output, short *input, int nb_samples)
{
    ...
    if (s->input_channels >= 2 &&
        s->output_channels == 1) {
        buftmp3[0] = output;
        multi_to_mono_split(buftmp2[0], input, nb_samples, s->input_channels);
