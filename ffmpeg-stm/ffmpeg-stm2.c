...
int64_t SegmentEnd(STMTranscodeSettings *settings)
{
	int64_t segment_end = 
...
int64_t BaseTime(STMTranscodeSettings *settings, int64_t stream_pts, AVRational stream_time_base)
{
	int64_t current_pts =
...
int in_memory_url_open(URLContext *h, const char *filename, int flags)
{
	h->priv_data = 0;
	return 0;
}
int in_memory_url_read(URLContext *h, unsigned char *buf, int size)
{
...
int in_memory_url_write(URLContext *h, unsigned char *buf, int size)
{
	if ((int)h->priv_data + size > inmemory_buffer_size)
	{
		size = inmemory_buffer_size - (int)h->priv_data;
...
int64_t in_memory_url_seek(URLContext *h, int64_t pos, int whence)
{
...
int in_memory_url_close(URLContext *h)
{
	return 0;
}

int in_memory_url_read_pause(URLContext *h, int pause)
{
	return 0;
}

int64_t in_memory_url_read_seek(URLContext *h, int stream_index, int64_t timestamp, int flags)
{
	return 0;
}

int in_memory_url_get_file_handle(URLContext *h)
{
	return (int)inmemory_buffer;
}

void register_inmemory_protocol()
{
...
void realloc_storage(void **pointer, size_t unit_size, size_t *capacity, size_t count)
{
...
void AddNewOutputCodec(STMTranscodeContext *context, AVCodec *codec)
{
...
void AddNewInputFile(STMTranscodeContext *context, AVFormatContext *file, int64_t ts_offset)
{
...
void AddNewOutputFile(STMTranscodeContext *context, AVFormatContext *file)
{
...
void AddNewStreamMap(STMTranscodeContext *context, AVStreamMap map)
{
...
void AddNewMetaDataMap(STMTranscodeContext *context, AVMetaDataMap map)
{
...
size_t FilenameSuffixLength()
{
	// Length of blah excluding %s when %d is expanded... "%s-%05d.ts"
	return 9;
}

void UpdateFilePathForSegment(STMTranscodeSettings *settings)
{
...
int process_segment_change(STMTranscodeSettings *settings, AVFormatContext *s, AVPacket *pkt)
{
...
char *newTitleForFile(AVFormatContext *input_file, int *isFilename)
{
...
char *newArtistForFile(AVFormatContext *input_file, int *isDirname)
{
	char *filename = input_file->filename;
	int filenameLength = strlen(filename);
...
int configureOutputFiles(STMTranscodeContext *context, char *filename)
{
...
char *newFileStringWithDifferentExtension(char *filename, char *ext)
{
	int i;
...
char *newFileStringWithAdditionalExtension(char *filename, char *ext)
{
	char *other;
...
char *newLanguageFromStreamSpecifier(char *specifier, long *indexOut)
{
...
int fileExistsWithDifferentExtension(char *filename, char *ext)
{
...
int fileExistsWithAdditionalExtension(char *filename, char *ext)
{
	char *other;
	asprintf(&other, "%s.%s", filename, ext);
	if (!other)
...
int isParseableSubtitleCodec(AVCodecContext *codec)
{
...
int writeJsonToFile(STMTranscodeContext *context, FILE *file)
{
	int isDirname;
	int isFilename;
...
int applyMetadataOutputSettings(STMTranscodeContext *context)
{
	int videoStreamIndex = -1;
	for (int i = 0; i < context->input_files[0]->nb_streams; i++)
	{
